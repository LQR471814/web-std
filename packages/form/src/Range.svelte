...
<div class={classList("flex", className)}>
  ...
  <div class="relative w-full">
    <button
      class="w-full py-3 hover:cursor-pointer"
      on:mousedown={(e) => {
        _dragging = true;
        onMouseMove(e);
      }}
      ...
      bind:this={_track}
    >
      <div class="flex items-center w-full h-fit rounded-full overflow-hidden">
        <div
          class={twMerge(
            "h-[8px] bg-slate-900 rounded-full",
        ...
        <div
          class={twMerge(
            "h-[5px] flex-1 bg-slate-900 rounded-full",
            disabled ? "bg-slate-700" : "",
            trackClass
          )}
          style={styleList({ width: _percentage })}
